-- Atomically modify an existing sale transaction and keep inventory/customer aggregates consistent.
-- Scope: sale transactions only.
...
DECLARE
  ...
  line_color TEXT;
  line_size TEXT;
  unregistered_line_allowed BOOLEAN;
...
BEGIN
  ...
  LOOP
    ...
    IF line_product_id IS NULL THEN
      -- Existing unregistered lines may remain/adjust/remove, but new ones cannot be created.
      SELECT EXISTS (
        SELECT 1
      ...
      ) INTO unregistered_line_allowed;

      IF NOT unregistered_line_allowed THEN
  ...
  paid_amount := COALESCE(tx_record.cash_amount, 0) + COALESCE(tx_record.transfer_amount, 0) + COALESCE(tx_record.card_amount, 0);
  IF new_total < paid_amount THEN
  ...
  old_unpaid := GREATEST(old_total - paid_amount, 0);
  new_unpaid := GREATEST(new_total - paid_amount, 0);
