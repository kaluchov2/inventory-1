-- Harden undo_sale_transaction for unregistered/legacy items.
-- Unregistered rows (null/blank/non-existent product_id) should not block undo.
...
DECLARE
  ...
  skipped_product_refs INTEGER := 0;
BEGIN
  ...
  -- Count non-empty product references that no longer exist in products.
  -- These are skipped for stock restore instead of hard-failing undo.
  SELECT COUNT(*)::INTEGER
  INTO skipped_product_refs
  FROM (
    SELECT DISTINCT NULLIF(BTRIM(ti.product_id::text), '') AS product_id_text
    FROM transaction_items ti
    WHERE ti.transaction_id = target_transaction_id
  ) refs
  LEFT JOIN products p ON p.id::text = refs.product_id_text
  WHERE refs.product_id_text IS NOT NULL
    AND p.id IS NULL;
  ...
  FOR stock_row IN
    ...
    WHERE ti.transaction_id = target_transaction_id
      AND NULLIF(BTRIM(ti.product_id::text), '') IS NOT NULL
  ...
  RETURN jsonb_build_object(
    ...
    'skippedProductRefs', skipped_product_refs
  );
END;
$$ LANGUAGE plpgsql;
