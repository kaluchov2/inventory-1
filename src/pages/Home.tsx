...
export function Home() {
  ...
  const totalProducts = filteredProducts.filter(
    (p) => p.availableQty > 0,
  ).length;
  ...
  return (
    <VStack spacing={{ base: 4, md: 6, lg: 8 }} align="stretch">
      ...
      <HStack spacing={3} flexWrap="wrap">
        <Text fontWeight="medium" color="gray.600" whiteSpace="nowrap">
          Filtrar por UPS:
        </Text>
        <Select
          size="sm"
          ...
          value={selectedUps}
          onChange={(e) =>
            setSelectedUps(e.target.value ? Number(e.target.value) : "")
          }
          borderRadius="md"
        >
      ...
      <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={6}>
        ...
        <StatCard
          title={
            es.dashboard.outstandingBalance +
            (selectedUps !== "" ? " (global)" : "")
          }
