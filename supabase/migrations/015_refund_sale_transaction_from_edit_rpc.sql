-- Register a refund from the Clientes sale editor without mutating the original sale.
...
BEGIN
  ...
  old_unregistered AS (
    SELECT
      'unreg:' || ti.product_name || '|' || ti.unit_price::text AS item_key,
  ...
  new_unregistered AS (
    SELECT
      'unreg:' || ei.product_name || '|' || ei.unit_price::text AS item_key,
      NULL::TEXT AS product_id,
  ...
  SELECT
    ...
    product_name,
    refund_qty,
    CASE WHEN refund_qty > 0 THEN refund_total / refund_qty ELSE 0 END AS unit_price,
    refund_total,
    category,
    brand,
    color,
