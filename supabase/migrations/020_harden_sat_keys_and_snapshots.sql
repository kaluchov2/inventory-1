ALTER TABLE public.products
  DROP CONSTRAINT IF EXISTS products_sat_key_id_fkey;

ALTER TABLE public.products
  ADD CONSTRAINT products_sat_key_id_fkey
  FOREIGN KEY (sat_key_id)
  REFERENCES public.sat_keys(id)
  ON UPDATE CASCADE
  ON DELETE SET NULL;

DROP INDEX IF EXISTS idx_sat_keys_code_unique;

ALTER TABLE public.transaction_items
  ADD COLUMN IF NOT EXISTS sat_key_id TEXT,
  ADD COLUMN IF NOT EXISTS sat_key_code TEXT,
  ADD COLUMN IF NOT EXISTS sat_key_description TEXT;

CREATE INDEX IF NOT EXISTS idx_transaction_items_sat_key_id
  ON public.transaction_items(sat_key_id);

CREATE OR REPLACE FUNCTION public.prevent_sat_key_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
BEGIN
  IF
    NEW.is_deleted = TRUE
    ...
    AND EXISTS (
      ...
      WHERE sat_key_id = OLD.id
        AND is_deleted = FALSE
      LIMIT 1
    )
  THEN
    RAISE EXCEPTION 'sat_key_in_use';
  END IF;
...
DROP TRIGGER IF EXISTS prevent_sat_key_soft_delete_if_in_use
  ON public.sat_keys;

CREATE TRIGGER prevent_sat_key_soft_delete_if_in_use
  BEFORE UPDATE ON public.sat_keys
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_sat_key_soft_delete_if_in_use();
