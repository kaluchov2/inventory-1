-- Update modify_sale_transaction to allow adding unregistered (UPS 0 / product_id null) lines during edit.
-- Preserves existing auto-settlement behavior for originally paid sales.
-- Keeps inventory/customer updates atomic and safe.
...
BEGIN
  ...
  LOOP
    ...
    -- Unregistered lines (product_id null) are allowed.
    -- They do not affect stock because inventory deltas only process non-null product_id rows.
