-- Fix SAT snapshot preservation when editing sales and allow SAT code reuse after soft delete.

ALTER TABLE public.sat_keys
  DROP CONSTRAINT IF EXISTS sat_keys_code_key;
...
DECLARE
  ...
  line_sat_key_code TEXT;
  line_sat_key_description TEXT;
  line_category TEXT;
  line_brand TEXT;
  line_color TEXT;
  line_size TEXT;
...
BEGIN
  ...
  LOOP
    ...
    line_sat_key_id := NULLIF(current_item->>'satKeyId', '');
    line_sat_key_code := NULLIF(current_item->>'satKeyCode', '');
    line_sat_key_description := NULLIF(current_item->>'satKeyDescription', '');
